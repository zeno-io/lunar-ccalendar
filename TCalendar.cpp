...
int MyCTCalendar::ctcl_solar_to_lunar(int y,int m,int d,struct CTCalendar* ctc)
{
  ...
  QString MD = QString::fromUtf8(buf);
  //qDebug()<<"MD:"<<MD.toInt();
  QString QueryStr;
...
bool MyCTCalendar::InitConnection(QString Path)
{
    ...
    if (!db.open()) {
       qDebug() << "warning: lunar.db doesn't exists.";
       return false;
    }
...
{
    QFile f("mccalendar.log" );
    if ( !f.open(QIODevice::ReadWrite | QIODevice::Append ) ){
//        qDebug()<< "Error time: " << QDateTime::currentDateTime().toString() << "\n";
//        qDebug() << Type << "\n";
//        qDebug() << error << "\n";
        return;
    }
